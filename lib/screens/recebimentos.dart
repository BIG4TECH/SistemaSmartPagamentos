...
class RecebimentosRelatorio extends StatefulWidget {
  final String email;
  const RecebimentosRelatorio(this.email);
...
class _RecebimentosRelatorioState extends State<RecebimentosRelatorio> {
  ...
  Future<void> _fetchRecebimentos() async {
    ...
    setState(() {
      recebimentos = snapshot.docs.map((doc) => doc.data() as Map<String, dynamic>).toList();
      totalValor = recebimentos.fold(0.0, (sum, item) => sum + (item['valor'] as double));
    });
  }
      ...
      pw.Page(
        build: (pw.Context context) => pw.Column(
          children: [
            ...
            pw.Text("Período: ${DateFormat('dd/MM/yyyy').format(selectedDateRange!.start)} - ${DateFormat('dd/MM/yyyy').format(selectedDateRange!.end)}"),
            pw.Text("Soma Total: R\$ ${totalValor.toStringAsFixed(2)}"),
            pw.SizedBox(height: 10),
            pw.Table.fromTextArray(
              headers: ['Valor', 'Email do Cliente', 'Produto', 'Tipo de Pagamento'],
              data: recebimentos.map((recebimento) {
                return [
                  recebimento['valor'].toString(),
                  recebimento['email_cliente'],
